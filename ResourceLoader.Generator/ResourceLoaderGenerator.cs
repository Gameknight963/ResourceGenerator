...
namespace ResourceLoader.Generator
{
    ...
    public sealed class ResourceLoaderGenerator : IIncrementalGenerator
    {
            ...
            string? projectDir)
        {
            ...
            bool recursive = resourceFolderAttr.ConstructorArguments.Length > 2 &&
                resourceFolderAttr.ConstructorArguments[2].Value is true;
            ...
            IEnumerable<(string FullPath, string RelativePath)> files = GetFiles(fullScanPath, recursive);
            string className = classSymbol.Name;
            string namespaceName = classSymbol.ContainingNamespace.ToDisplayString();

            Dictionary<string, List<(string FullPath, string FileName)>> filesByDir = new();
            foreach ((string fullPath, string relativePath) in files)
            {
                string dir = Path.GetDirectoryName(relativePath) ?? string.Empty;
                string fileName = Path.GetFileName(relativePath);
                if (!filesByDir.ContainsKey(dir))
                    filesByDir[dir] = new List<(string, string)>();
                filesByDir[dir].Add((fullPath, fileName));
            }

            System.Text.StringBuilder properties = new();
            HashSet<string> usedLoaders = new();
            Dictionary<string, string> usedPropertyNames = new();

            if (filesByDir.TryGetValue(string.Empty, out List<(string FullPath, string FileName)>? topFiles))
                EmitFiles(topFiles, properties, usedLoaders, usedPropertyNames, loaderMap, runtimePath, ctx, scanPathLocation, indent: "    ");

            foreach (KeyValuePair<string, List<(string FullPath, string FileName)>> dir in filesByDir)
            {
                if (dir.Key == string.Empty) continue;
                string className2 = SanitizeName(dir.Key);
                properties.AppendLine($"    public static class {className2}");
                properties.AppendLine("    {");
                Dictionary<string, string> nestedPropertyNames = new();
                EmitFiles(dir.Value, properties, usedLoaders, nestedPropertyNames, loaderMap, runtimePath, ctx, scanPathLocation, indent: "        ");
                properties.AppendLine("    }");
                properties.AppendLine();
            }
            ...
            {
                string loaderFieldName = $"__rl_{GetLoaderFieldName(loaderTypeName)}";
                loaderFields.AppendLine($"    [global::System.Runtime.CompilerServices.CompilerGenerated]");
                loaderFields.AppendLine($"    [global::System.ComponentModel.EditorBrowsable(global::System.ComponentModel.EditorBrowsableState.Never)]");
                loaderFields.AppendLine($"    private static readonly {loaderTypeName} {loaderFieldName} = new {loaderTypeName}();");
            }
        ...
        private static void EmitFiles(
            List<(string FullPath, string FileName)> files,
            System.Text.StringBuilder properties,
            HashSet<string> usedLoaders,
            Dictionary<string, string> usedPropertyNames,
            Dictionary<string, LoaderInfo> loaderMap,
            string runtimePath,
            SourceProductionContext ctx,
            Location scanPathLocation,
            string indent)
        {
            foreach ((string fullPath, string fullFileName) in files)
            {
                string extension = Path.GetExtension(fullFileName).ToLowerInvariant();
        ...
        private static IEnumerable<(string FullPath, string RelativePath)> GetFiles(
            string rootPath,
            bool recursive)
        {
            ...
            foreach (string dir in Directory.GetDirectories(rootPath))
            {
                ...
                foreach ((string fullPath, string relativePath) in GetFiles(dir, recursive: true))
                    yield return (fullPath, Path.Combine(dirName, relativePath));
            }
        }
