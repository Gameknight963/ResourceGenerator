    ...
    public sealed class ResourceLoaderGenerator : IIncrementalGenerator
    {
            ...
            string indent)
        {
            foreach ((string fullPath, string fullFileName) in files)
            {
                ...
                string loaderField = $"_{GetLoaderFieldName(loader.LoaderTypeName)}";
                properties.AppendLine($"{indent}private static {loader.ReturnTypeName}? {backingFieldName};");
                properties.AppendLine($"{indent}public static {loader.ReturnTypeName} {fieldName} =>");
                properties.AppendLine($"{indent}    {backingFieldName} ??= {loaderField}.Load(System.IO.Path.Combine({runtimePath}, \"{fullFileName}\"));");
                properties.AppendLine();
            }
        }
